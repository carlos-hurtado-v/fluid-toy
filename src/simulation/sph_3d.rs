//! 3D SPH (Smoothed Particle Hydrodynamics) simulation module

use crate::simulation::particle::SphParticle3D;
use crate::state::{GpuBoundsParams3D, GpuSphParams3D};
use wgpu::util::DeviceExt;

const WORKGROUP_SIZE: u32 = 64;

/// 3D SPH simulation with 3-pass compute pipeline
...
impl SphSimulation3D {
    pub fn new(
        ...
        let density_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("SPH 3D Density Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/sph_density_3d.wgsl").into()),
        });

        let force_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("SPH 3D Force Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/sph_force_3d.wgsl").into()),
        });
        ...
        let particle_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("SPH 3D Particle Buffer"),
        ...
        let sph_params_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("SPH 3D Params Buffer"),
        ...
        let bounds_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Bounds 3D Params Buffer"),
        ...
        let density_force_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("SPH 3D Density/Force Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    ...
                    ty: wgpu::BindingType::Buffer {
                        ...
                        min_binding_size: wgpu::BufferSize::new(
                            std::mem::size_of::<GpuSphParams3D>() as u64,
                        ),
                    },
                    count: None,
                },
        ...
        let integrate_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("SPH 3D Integrate Bind Group Layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::COMPUTE,
                ...
                wgpu::BindGroupLayoutEntry {
                    ...
                    ty: wgpu::BindingType::Buffer {
                        ...
                        has_dynamic_offset: false,
                        min_binding_size: wgpu::BufferSize::new(
                            std::mem::size_of::<GpuBoundsParams3D>() as u64,
                        ),
                    },
                    count: None,
                },
        ...
        let density_pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("SPH 3D Density Pipeline Layout"),
        ...
        let density_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("SPH 3D Density Pipeline"),
        ...
        let density_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("SPH 3D Density Bind Group"),
        ...
        let force_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("SPH 3D Force Bind Group"),
            layout: &density_force_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let integrate_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("SPH 3D Integrate Bind Group"),
    ...
    /// Each pass is in a separate compute pass to ensure proper synchronization
    pub fn step(&self, encoder: &mut wgpu::CommandEncoder) {
        // Pass 1: Compute density
        {
            let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("SPH 3D Density Pass"),
        ...
        {
            let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("SPH 3D Integrate Pass"),
                timestamp_writes: None,
            });
