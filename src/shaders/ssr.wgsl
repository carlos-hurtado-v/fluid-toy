...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Read smooth world-space normal from G-buffer (written during front face pass)
    let normal_sample = textureLoad(normal_tex, pixel, 0);
    if (normal_sample.a < 0.5) {
        // No normal written — shouldn't happen if depth check passed, but safety
        textureStore(ssr_output, pixel, vec4<f32>(0.0));
        return;
    }
    let world_normal = normalize(normal_sample.xyz);

    // Transform world normal to view space
    let normal_view = normalize((camera.view * vec4<f32>(world_normal, 0.0)).xyz);
